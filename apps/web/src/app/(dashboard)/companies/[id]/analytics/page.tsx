// apps/web/src/app/(dashboard)/companies/[id]/analytics/page.tsx
'use client';
...
export default function CompanyAnalyticsPage() {
  ...
  const syncAnalytics = async (forceRefresh = false) => {
    ...
    try {
      const res = await fetch('/api/analytics/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ companyId, forceRefresh }),
      });
  ...
  useEffect(() => {
    ...
    const fetchPlatforms = async () => {
      ...
      try {
        const res = await fetch(`/api/platforms?companyId=${companyId}`, { signal });
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    if (companyId) fetchSyncStatus();
  }, [companyId, fetchSyncStatus]);

  // Reset insights when filters change
  useEffect(() => {
    setAiInsights(null);
  }, [selectedPlatformType, dateRange]);
  ...
  const buildParams = useCallback(
    ...
    [companyId, selectedPlatformType, getDateRange]
  );
  ...
  }, [buildParams, granularity, companyId]);

  useEffect(() => {
    if (!companyLoading && companyId) fetchAnalytics();
  }, [fetchAnalytics, companyLoading, companyId]);
