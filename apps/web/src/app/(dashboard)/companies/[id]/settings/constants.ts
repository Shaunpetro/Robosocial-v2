...
/**
 * Parse preferredTimes into an array of time slot IDs (e.g., ["morning","afternoon"]).
 * Handles:
 * - Array of slot IDs: ["morning","afternoon"]
 * - Array of time strings: ["09:00","14:00"]
 * - Object of day -> times: { "Monday": ["09:00"], "Wednesday": ["14:00"] }
 * Returns fallback slot IDs if no valid slots found.
 */
export function parsePreferredTimes(times: unknown): string[] {
  ...
  if (Array.isArray(times)) {
    times.forEach(t => {
      if (typeof t === 'string') addSlot(t)
    })
  } else if (times && typeof times === 'object') {
    // times is object: { Monday: ['09:00'], ... }
    Object.values(times as Record<string, unknown>).forEach(val => {
  ...
  if (slotIds.length === 0) {
    return ['morning', 'afternoon']  // safe default
  }
