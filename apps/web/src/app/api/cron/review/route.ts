...
export async function GET() {
  ...
  console.log('[Review] ========================================');
  console.log('[Review] Starting period-aware performance review...');
  ...
  try {
    ...
    for (const company of companies) {
      ...
      try {
        ...
        const lastUpdate = intel.lastIntelligenceUpdate;
        const now = new Date();
        ...
        console.log(`[Review] Analyzing posts from ${startDate.toISOString()} to ${endDate.toISOString()} for ${period}`);
        ...
        if (nonEngagingCount > 0) {
          result.recommendations.push(
            `${nonEngagingCount} posts had low engagement (<5 likes and <5 comments). Consider adjusting topics or content types.`
          );
        }
        ...
        result.avgEngagementRate = Math.round((totalEngagement / analysisPosts.length) * 100) / 100;

        const postsWithKeys = analysisPosts.map(post => {
        ...
        result.recommendations.push(...generateRecommendations(
          dayPerformance,
