...
function buildInsightsPrompt(data: AnalyticsData): string {
  ...
  if (Object.keys(byPlatform).length > 0) {
    ...
    for (const [platform, metrics] of Object.entries(byPlatform)) {
      prompt += `### ${platform}\n`;
      prompt += `- Posts: ${metrics.posts}\n`;
      prompt += `- Impressions: ${metrics.impressions.toLocaleString()}\n`;
  ...
  if (topPosts && topPosts.length > 0) {
    ...
    topPosts.slice(0, 3).forEach((post, idx) => {
      ...
      const engRate = post.impressions > 0 ? ((totalEng / post.impressions) * 100).toFixed(2) : "0";
  ...
  if (timing) {
    ...
    if (timing.bestHours && timing.bestHours.length > 0) {
      const hours = timing.bestHours.slice(0, 3).map(h => h.label).join(", ");
  ...
  if (trends && trends.length > 1) {
    ...
    const engagementChange = lastPeriod.engagement - firstPeriod.engagement;
    const trend = engagementChange > 0 ? "increasing" : engagementChange < 0 ? "decreasing" : "stable";
    prompt += `## TREND\n`;
    prompt += `- Engagement is ${trend} over the period\n`;
    prompt += `- Change: ${engagementChange > 0 ? "+" : ""}${engagementChange} engagements\n\n`;
  }
